# Where a contract advertises its interface

A reader that holds only a contract's address needs the commitment and the `index.json` URL of the contract's interface bundle, then verifies the bundle with the three levels described in the [README](../README.md). This repository delivers one way to publish them: the public-interface event. A contract has one interface, at one authoritative URL, and the newest event wins.

While the design was open, five other places were implemented, tested and tried on Stagenet. They are described below as alternatives studied, not delivered: their code is no longer in the repository. Every number on this page was measured, except those marked as modelled or computed from a cost model.

Measured with `compact` 0.34.0, `@midnight-ntwrk/compact-runtime` 0.19.0 and Ledger v9 (Midnight Stagenet).

## The public-interface event (delivered)

**Where it lives.** `publishBundle(payload)`, from `compact/OffChainInterface.compact`, emits one `Misc` event. Its name is fixed inside the circuit, so no caller can choose another; [FORMAT.md](FORMAT.md#event) gives the exact name. The payload is the 00021 layout: bytes 0 to 31 are the bundle's commitment, and bytes 32 to 255 are the URL of its `index.json`, zero padded.

**How to find it.** `contractEvents(filter: { contractAddress, types: [MISC] })`, keeping only the events whose 32-byte name is exactly that name, zero padded. The newest one wins. `verify --indexer <url> --address <hex>` reads it and prints the ids of the events it supersedes. Every other `Misc` event is ignored, including one named `bundle/v1`, the event's previous name, and the per-standard names of the alternatives below.

**Key effect.** None. The circuit reads no ledger field, so its verifier key is the same in every contract: identical in the three examples. Renaming the event changed this one key and nothing else: every other key of the example contracts, and every file compiled from the interfaces, stayed byte-identical.

**Cost.** A 67,427,634-byte prover key (67,427,609 bytes under the previous name), because the 256-byte payload is decomposed byte by byte. One transaction with one proof per bundle version. Modelled bytes written: 530, all deleted again, for a 61-byte URL, so an event changes no contract state. The figure depends on the payload's length, not on the name: it is the same under the previous name.

**Who can write it.** Anyone who can call the contract, unless the contract restricts `publishBundle`. The README's How to use shows a commented check that does.

**Caveats.**
- One interface per contract. Its bundle publishes every circuit the contract makes readable.
- It needs an indexer with the contract-event API (4.4.0 or later, marked beta). A consumer that reads only the contract state does not see it.
...
Each alternative held one entry per standard, for example `erc20` and `erc20-metadata`. None of them is delivered. Their code, the discovery tool that read them (`src/discover.mjs`), `verify --standard` and their tests are in commit `90ad944` and were removed after it. Paths, commands and tool behaviour in this section are those of `90ad944`. In the tables, P0 is the delivered event under its previous name, `bundle/v1`, which the alternatives treated as the unnamed default standard.

### One name, one value

Every placement used the same name and the same value.

| | |
|---|---|
| Name | `iface/v1/<standard>`, ASCII, at most 32 bytes, so `<standard>` is at most 23 bytes. Where 32 bytes are needed it is zero padded: `pad(32, "iface/v1/erc20")`. `bundle/v1`, the event's previous name, was the unnamed default standard. |
| Value | `InterfaceRef { commitment: Bytes<32>; url: Opaque<"string">; }`: the bundle's 32-byte commitment and the URL of its `index.json`. |

Readers recognised entries by the `iface/v1/` prefix, so no separate marker was needed.

At `90ad944`, `src/discover.mjs` listed every entry with the placement it came from, from the indexer or from a state alone, and needed only `@midnight-ntwrk/compact-runtime` and `fetch`. `verify --standard <name>` took the first placement that held the standard, in this order: operations metadata, the spare root slot `[15]`, registry at the start of the ledger, registry at the end, newest event. For a compactc contract, the order follows the strongest write restriction a reader can rely on without knowing the contract: the maintenance authority, the deployer at deploy time, contract logic that may be ungated, then anyone who can call an emitting circuit. The exception is a contract with a circuit that writes `[15]`: one the maintenance authority adds (footnote 2), or one written with MinoCrab (see [MinoCrab](#minocrab)). Such a circuit makes `[15]` as open as the circuit itself, and the order still preferred `[15]`. It printed the placement it used and warned when another placement held a different entry for the same standard.

The order decided which entry was checked, not whether to trust it: an entry is a claim by whoever could write it, and the three levels ran the same way on every entry.
...
(2) No compactc circuit can address root index 15, and maintenance updates change operations, not state. But a maintenance authority can add a circuit, and a circuit written with MinoCrab can read and write `[15]` (see [MinoCrab](#minocrab)), so a P5 entry is fixed only once the authority is frozen or absent. A maintenance authority can also supersede a P5 entry with a P2 entry for the same standard, which discovery preferred.
...
(4) A registry value is one ledger cell, and Ledger v9 rejects a state with a cell over 32,768 bytes when it deserializes it. With a 32-byte commitment that leaves 32,726 bytes for the URL, measured on the P5 map; `src/slot15.mjs` refused more. P3 and P4 store the same cell, but writing one that large through `publishInterface` was not tried.

"Modelled bytes written" is the `gasCost` the runtime reports for a local call with its default cost model, on the populated registry examples. The event circuits wrote and deleted the same 539 bytes for the URL used there (the figure grows with the URL), so an event changes no contract state. A second registry entry costs 1,932 bytes (+530), an update 1,940 (+8), and a removal 1,410 (−530). Stagenet's cost model may differ.

### P1 — one event per standard (not delivered)
...
**How to find it.** The same event query as P0. The newest event per name won, and `discover` listed the ids it superseded.
...
**Caveats.** The same as P0. The caller assembled both the name and the payload, because Compact has no byte concatenation.

**On Stagenet.** Added to the deployed 00021 contract by maintenance (footnote 1), then used once: `iface/v1/erc20-metadata`, event id 42945, which verified to Level 3.

**Why it was dropped.** It is the delivered event with the standard in its name. With one interface per contract, the fixed name is enough, and one circuit does the job of two.

### P2 — operations metadata (not delivered)
...
**Update authority.** Only the maintenance authority, which makes this the one placement that is restricted without extra code. It is also why `verify --standard` preferred it.

**Caveats.** It uses operation IR for data, which is not what IR is for. A future Midnight version that validates IR could reject such entries or remove the ability to add them. Entry point names are public. Tests built these operations locally with a helper in `test/helpers.mjs` that reproduced the live operation byte for byte; real writes used ledger-v9 `IrInsert`.
...
### P3 — registry at the start of the ledger (not delivered)
...
**Key effect.** Every other field moves up one position. In the example, all six ERC-20 reads get new keys, so the registry-first contract needs its own interface (`examples/registry-first/Interface.compact`), which imports the same two modules in the same order. `check-keys` confirmed it matched its contract. Above 15 fields the effect is partial (see "Layout rules").
...
- There is no access control. The commented check in the template compiled once uncommented, and a test checked this.

### P5 — spare slot 15 (not delivered)

**Where it lives.** Index 15 of the state's root array. compactc puts at most 15 entries in any array, so its root has at most 15 entries in every contract, whatever the field count. Ledger v9 allows 16 entries per array and checks that limit when a state is deserialized. The deployer builds the initial state with the unused root indices set to `null` and a registry map at `[15]`, in the same encoding as P3 and P4. `src/slot15.mjs` provided `withSpareSlotRegistry(state, refs)`, which returned the patched state, and `interfaceMapValue(refs)`, which returned the map on its own. Both need only `@midnight-ntwrk/compact-runtime`. The map is built with the runtime type descriptors that compactc's generated code uses. A test checked that it was byte-identical to the map `publishInterface` writes for the same entries.
...
**How to find it.** `[15]` is the root's last entry, so it is the state's last leaf, where discovery already looked. `discover` reported the entries as `ledger-last` with `spareSlot: true`, and `verify --standard` ranked them second: after operations metadata, before both ledger registries. `discover` and `verify` printed `spare slot [15]`. The label is exact, because only a deployer-extended root has a 16th entry.

**Key effect.** None, for any number of fields. No circuit changes, and no compactc circuit reads or writes `[15]`: `check-keys` still reported 25 IDENTICAL. On the extended state, the fungible example's seven reads give the same results as on the original state, and a write circuit keeps the root at 16 entries. The same holds for reads of field 0 and field 19 of a 20-field contract, whose root `[[5],[15]]` is padded to 16 entries. Contracts with 15 fields and with no fields were also tested.
...
**On Stagenet.** `6bd2c5be…7bbd4a` was deployed this way with two standards (block 587459), and an ordinary midnight-js session then proved and submitted `totalSupply()` on it (block 587463). Both entries verified to Level 3 from the public indexer. The deploy script was `live/stagenet/deploy.mjs slot15`.
...
- The JavaScript binding's `StateValue.arrayPush` refuses a 16th entry, so the helper builds the root with `StateValue.decode`, which the runtime types as internal. The binding also builds a 17-entry array in memory, but `ContractState.deserialize` rejects it. The helper refused a root that already had 16 entries, and an entry whose cell would exceed the ledger's 32,768-byte bound, which `StateValue.decode` does not check.
- `verify --standard` ranked `[15]` above both ledger registries, so a contract that also kept an updatable registry always got its `[15]` entry for a standard both held. Such a standard had to be updated through operations metadata instead.
- Combined with P4, the P4 map is no longer the last leaf, so discovery reported only `[15]`.

### Other places considered

These were studied and not implemented.
...
These are the rules the ledger placements depend on. They were measured on generated contracts and checked by `test/placement-layout.test.mjs` at `90ad944`. The rule that keys depend on slot paths still matters for every interface; [FORMAT.md](FORMAT.md#partial-source) states the part the delivered design relies on.
...
At `90ad944`, `src/registry.mjs` implemented the steps below. The tests ran them on both registry examples, on generated contracts of 20 and 250 fields, on the other examples, and on two captured Stagenet states.
...
2. **Ledger, first and last.** From the root of the state, descend first (or last) children while the value is an array. A one-field ledger has one leaf, and it was reported once.
...
5. **Events.** Take `Misc` events named `bundle/v1` or `iface/v1/<standard>`. The newest per name won.

A name longer than 23 bytes after the prefix, a prefixed name with a zero, control or non-ASCII byte, a prefixed key whose value is not an `InterfaceRef`, an `iface/v1/` entry point without the blob, or a URL that is not a single-line `http(s)` URL of printable ASCII was reported under "ignored" and never returned as an entry. On `examples/fungible`, `examples/nft` and `examples/multi`, discovery found nothing. The last field of `examples/nft` is a populated map; discovery looked at it and rejected it.

### Evidence on Stagenet

Two Stagenet states were stored under `test/fixtures/` and decoded by `test/operations.test.mjs` at `90ad944`. The first is still in the repository, where the verifier's regression tests use it as a real contract state:
...
Six contracts on Stagenet carry entries. Each was checked with `discover` and `verify --standard` at `90ad944`, and every transaction is recorded in `live/stagenet/deployment.json`. The bundles are still served from the copies in `live/stagenet/site/`.
...
All twelve entries verified to Level 3 from the public indexer with the verifier of `90ad944`. A bundle does not depend on the contract's address, so the two hosted for `2f4f7e6f` serve `84a104e1` and `6bd2c5be` unchanged. On 22 unrelated Stagenet contracts, four of which have a map as their first field, discovery reported nothing.

## MinoCrab

MinoCrab is a Rust library for writing Midnight circuits; it emits ZKIR v3 and uses the `zkir-v3` tool shipped with `compact` 0.34.0 for keys. It can express every placement above. Measured at MinoCrab `99504f0`, with the event under its previous name, `bundle/v1`:
...
**On Stagenet.** `5d82194f…ea0692` is `live/stagenet/contracts/ERC20Live.compact` compiled with `--feature-zkir-v3`, with MinoCrab's `publishBundle` in place of compactc's (`deploy.mjs minocrab-*` at `90ad944`). The chain holds MinoCrab's key for `publishBundle` (`813b25cc…`) and compactc's v3 keys for the six reads. midnight-js deployed it (block 587808) and proved and submitted `publishBundle` with MinoCrab's 1.77 MB key (block 587848, `SucceedEntirely`). Proving it took 0.3 s cold and about 0.2 s warm on the proof server, against 7.0 s cold and about 2.7 s warm with compactc's 56.6 MB v3 key (measured offline). The bundle ships the v3 keys and records `--feature-zkir-v3` in its `package.json`, so Level 3 recompiles with that flag; it verified to Level 3. The repository still builds and verifies such ZKIR v3 bundles. The proof-server image is the stock `midnightntwrk/proof-server:9.0.0-rc.6`.
