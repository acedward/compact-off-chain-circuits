...
contract advertises a 32-byte commitment to `index.json` and the URL of
that `index.json`, in a `bundle/v1` event or in one of the places the
verifier's `--standard` option reads.

| | |
|---|---|
| URL | `https://compact-off-chain-circuits.pages.dev/minocrab/erc20/index.json` |
| Contract address | `5d82194fac77216360bb4be5f3879007b46858877df6d9a2c7769d2e96ea0692` |
| Indexer | `https://indexer.stagenet.shielded.tools/api/v4/graphql` |
| Published circuits | `name`, `symbol`, `decimals`, `totalSupply`, `balanceOf`, `allowance` |
| Compiler / language / runtime | 0.34.0 / 0.26.0 / 0.19.0 (`--feature-zkir-v3`) |
...
```sh
node <compact-off-chain-circuits>/src/verify.mjs --bundle-url https://compact-off-chain-circuits.pages.dev/minocrab/erc20/index.json \
  --indexer https://indexer.stagenet.shielded.tools/api/v4/graphql --address 5d82194fac77216360bb4be5f3879007b46858877df6d9a2c7769d2e96ea0692 --circuit name --args ...
```

Without `--bundle-url` the verifier takes the URL from the contract's latest
event, or with `--standard <name>` from that standard's entry. It downloads `index.json`, checks it against the commitment on chain,
...
Add `--level 3` to recompile `src/integrations/openzeppelin/FungibleTokenReadable.Interface.compact` with compact
0.34.0 and `--feature-zkir-v3` and check that it reproduces the shipped keys and
