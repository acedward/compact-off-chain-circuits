#!/usr/bin/env node
// For every token: each circuit published by the interface build must have a
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  for (const r of rows) console.log(`${r.status.padEnd(9)} ${r.token}/${r.circuit}${r.bytes ? ` (${r.bytes} bytes)` : ''}`);
